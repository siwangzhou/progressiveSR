...
class MyDataset(Dataset):
    ...
    def __getitem__(self, index):
        hrsize = 64
        lrsizes = [16, 18, 20, 22, 24, 26, 28, 30]
        ...
        if flip_ran == 0:
            # horizontal
            hr = torch.flip(hr, [1])
            lr_images = [torch.flip(lr, [1]) for lr in lr_images]
        elif flip_ran == 1:
            # vertical
            hr = torch.flip(hr, [2])
            lr_images = [torch.flip(lr, [2]) for lr in lr_images]
        ...
        if rot_ran != 0:
            ...
            lr_images = [torch.rot90(lr, rot_ran, [1, 2]) for lr in lr_images]

        hr = (hr - 0.5) * 2.0
        lr_images = [(lr - 0.5) * 2.0 for lr in lr_images]

        return lr_images, hr

class Test(Dataset):
    def __init__(self, alist=["D:/DYQ/DATASETS/Set5/HR_crop32"]):
    ...
    def __getitem__(self, index):
        scales = 32
        resize_factors = [8, 9, 10, 11, 12, 13, 14, 15]
        ...
        for factor in resize_factors:
            lrsize = (w * factor, h * factor)
            cropimg = sourceImg.resize(lrsize, Image.BICUBIC)
            lr = transforms.ToTensor()(cropimg)
            lr_images.append(lr)
