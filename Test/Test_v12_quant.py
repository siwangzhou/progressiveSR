...
import torch
from ops.OmniSR import LR_SR_x4_v12_quant
...
chpoint= torch.load("train_log_v12_quant/CS_Omni_P2P_x4/CS_Omni_P2P_x4_1275.pt")
...
for test_batch, (_, hrimg) in enumerate(test_data, start=0):
    ...
    if flag_visual:
        save_image(i1, './out_v12/' + str(index) + 'hr.png')
        save_image(i2, './out_v12/' + str(index) + 'sr.png')
        save_image(i3, './out_v12/' + str(index) + 'lr.png')
...
if flag_metric:
    ...
    fp = open('out_v12/test_log.txt', 'a+')
    fp.write(str_write)
    fp.close()