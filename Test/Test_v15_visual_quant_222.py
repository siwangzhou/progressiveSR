...
import torch
from ops.OmniSR_visual_quant_222 import LR_SR_x4_v15_quant
import numpy as np
from skimage.metrics import structural_similarity as ssim_get
from data_v14v15 import Test
...
chpoint= torch.load("train_log_v15_visual_quant_222/CS_Omni_P2P_x4/CS_Omni_P2P_x4_1014.pt")
...
for test_batch, (_, hrimg) in enumerate(test_data, start=0):
    ...
    if flag_visual:
        save_image(i1, './results_diff_visual_quant_222/out_v15_div2k/' + str(index) + 'hr.png')
        save_image(i2, './results_diff_visual_quant_222/out_v15_div2k/' + str(index) + 'sr.png')
        save_image(i3, './results_diff_visual_quant_222/out_v15_div2k/' + str(index) + 'lr.png')
...
if flag_metric:
    ...
    fp = open('out_v15/test_log.txt', 'a+')
    fp.write(str_write)
    fp.close()
